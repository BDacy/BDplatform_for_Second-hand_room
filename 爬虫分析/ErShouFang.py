...
#                          names=['总价', '平米单价', '小区', '所在区', '户型', '楼层', '面积', '户型结构', '建筑类型', '朝向', '建筑结构',
#                                 '装修情况',
...
def set_parameters():
    ...
    fieldnames = ['index', 'zong_taoshu', 'zong_jia', 'jiage_meiping', 'wei_zhi_0', 'wei_zhi_1', 'fangwu_huxing',
...
def crawl_7_day():
    index_num = len(pandas.read_csv(path))
    for i in range(1, 101):
        print('爬取第%d页' % i)
        url = 'https://cd.lianjia.com/ershoufang/pg%dco32tt2/' % i
        response = requests.get(url=url, headers=parameters['headers'])
        page_text = response.text
        tree = etree.HTML(page_text)
        zong_taoshu = tree.xpath('//h2[@class="total fl"]/span/text()')
        ...
        for li in li_list:
            ...
            dic = {
                'index': index_num,
                'zong_taoshu': zong_taoshu[0].strip(), 'zong_jia': dic['zong_jia'][0],
